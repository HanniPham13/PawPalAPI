  ...
  try {
    ...
    if (data.taggedPetIds && data.taggedPetIds.length > 0) {
      ...
      if (taggedPets.length !== data.taggedPetIds.length) {
        throw new Error(
          "One or more tagged pets not found or don't belong to you"
        );
      }
    }
    ...
    const postWithFollowingStatus = {
      ...post,
      isFollowing,
      isVerified: ["VERIFIED", "PURRPARENT", "SUPER_ADOPTER", "VET"].includes(
...
export const applyForPurrparentVerification = async (
  userId: string,
  documents: {
    documentType:
      | "ID_CARD"
...
export const createPetAdoptionPost = async (
  userId: string,
  data: {
    title: string;
    description: string;
  ...
  try {
    ...
    if (data.petProfileId) {
      ...
      if (!petProfile) {
        throw new Error(
          "Pet not found or you don't have permission to create an adoption post for this pet"
        );
      }
...
export const getUserAdoptionPosts = async (
  userId: string,
  page: number = 1,
  limit: number = 10
): Promise<{
...
export const getAdoptionPost = async (
  postId: string
): Promise<{
  success: boolean;
  message: string;
  ...
  try {
    const post = await prisma.petAdoptionPost.findUnique({
      where: { id: postId },
      include: {
        author: {
          select: {
    ...
    return {
      success: true,
      message: "Adoption post retrieved successfully",
      data: post,
    };
  } catch (error) {
    console.error("Get adoption post error:", error);
    return {
      success: false,
      message: "Failed to get adoption post details",
    };
  }
};

export const deletePetAdoptionPost = async (
  userId: string,
  postId: string
): Promise<{
  success: boolean;
  ...
  try {
    ...
    if (!post) {
      throw new Error(
        "Adoption post not found or you don't have permission to delete it"
      );
    }
...
export const applyForAdoption = async (
  ...
  try {
    ...
    if (
      user.verificationLevel !== "PURRPARENT" &&
      user.verificationLevel !== "SUPER_ADOPTER" &&
      user.verificationLevel !== "VET"
    ) {
      throw new Error(
        "You must be a verified Purrparent to apply for adoption"
      );
    }
    ...
    const chatRoom = await prisma.chat.create({
      data: {
        ...
        participants: {
          create: [{ userId: adoptionPost.authorId }, { userId: userId }],
        },
      },
    });
    ...
    await prisma.notification.create({
      data: {
        ...
        message: `${user.firstName} ${user.lastName} has applied to adopt ${
          adoptionPost.petProfile?.name || "your pet"
        }`,
...
export const getMyAdoptionApplications = async (
  ...
  try {
    ...
    const applicationsToReturn = hasMore
...
export const getReceivedAdoptionApplications = async (
  userId: string,
  page: number = 1,
  limit: number = 10
): Promise<{
