...
export const handleCreatePetProfile = async (
  req: AuthRequest,
  res: Response
): Promise<void> => {
  try {
    ...
    const profilePicture = req.file
      ? req.file.path
          .replace(/^C:\\xampp\\htdocs\\PawPalAPI-crap\\/, "")
          .replace(/\\/g, "/")
      : undefined;
    ...
    if (!name || !species) {
      res
        .status(400)
        .json({ success: false, message: "Pet name and species are required" });
      return;
    }
    ...
    if (!isValidEnum(species, PetSpecies)) {
      res.status(400).json({
        success: false,
        message: `Invalid species. Must be one of: ${Object.values(
          PetSpecies
        ).join(", ")}`,
      });
      return;
    ...
    if (gender && !isValidEnum(gender, PetGender)) {
      res.status(400).json({
        success: false,
        message: `Invalid gender. Must be one of: ${Object.values(
          PetGender
        ).join(", ")}`,
      });
      return;
    ...
    if (age !== undefined && (isNaN(Number(age)) || Number(age) < 0)) {
      res
        .status(400)
        .json({ success: false, message: "Age must be a positive number" });
      return;
    }
    ...
    res.status(500).json({
      success: false,
      message: "Server error while creating pet profile",
    });
  }
};

export const handleUpdatePetProfile = async (
  req: AuthRequest,
  res: Response
): Promise<void> => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Server error while updating pet profile",
    });
  }
};

export const handleDeletePetProfile = async (
  req: AuthRequest,
  res: Response
): Promise<void> => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Server error while deleting pet profile",
    });
  }
};

export const handleGetPetProfile = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Server error while retrieving pet profile",
    });
  }
};

export const handleGetUserPets = async (
  req: AuthRequest,
  res: Response
): Promise<void> => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Server error while retrieving user pets",
    });
  }
};

export const handleToggleAdoptableStatus = async (
  req: AuthRequest,
  res: Response
): Promise<void> => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Server error while updating pet adoption status",
    });
  }
};
...
export const handleAddMedicalRecord = async (
  req: AuthRequest,
  res: Response
): Promise<void> => {
  try {
...
export const handleUpdateMedicalRecord = async (
  req: AuthRequest,
  res: Response
): Promise<void> => {
  try {
    ...
    if (!petId || !recordId) {
      res
        .status(400)
        .json({ success: false, message: "Pet ID and record ID are required" });
      return;
    }
...
export const handleDeleteMedicalRecord = async (
  req: AuthRequest,
  res: Response
): Promise<void> => {
  try {
...
export const handleGetMedicalRecords = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
...
export const handleGetMedicalRecord = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
